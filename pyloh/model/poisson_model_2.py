...
class PoissonLatentVariables(LatentVariables):
    ...
    def update(self, parameters, iters):
	    ...
	    #print(str(j))
            if LOH_status_j == 'NONE':
                xi_j, psi_j = self._sufficient_statistics_by_segment_NL(parameters, eta, j)
            ...
            #if j == 0:
            #    print("xi: "+str(xi_j.shape))
            #    print("psi: "+str(psi_j.shape))

        self.sufficient_statistics = sufficient_statistics

    def _sufficient_statistics_by_segment_NL(self, parameters, eta, j):
        a_T_j = np.array([100])
        b_T_j = np.array([100])
        d_T_j = a_T_j + b_T_j
        I_j = d_T_j.shape[0]
        ...
        psi_j = self._get_psi_by_segment(D_N_j, D_T_j, phi, rho_j, eta)
    ...
    def _get_xi_by_segment(self, b_T_j, d_T_j, I_j, phi, eta):
        ...
        for c in range(0, C):
            xi_j[:, c, :] = np.log(p_CG[c]) + log_binomial_likelihood(b_T_j, d_T_j, mu_E_c)

        temp = np.sum(xi_j,axis=2)
        for i in range(0, I_j):
            for c in range(0, C):
                xi_j[i,c,:] = xi_j[i,c,:] / temp[i,c]
        xi_j += eps

        return xi_j

    def _get_psi_by_segment(self, D_N_j, D_T_j, phi, rho_j, eta):
        C = self.config_parameters['copynumber_tumor_num']
        G = self.config_parameters['genotypes_tumor_num']
        c_N = np.array(constants.COPY_NUMBER_NORMAL)
        c_T = np.array(self.config_parameters['copynumber_tumor'])
        ...
        psi_j = np.log(rho_j) + log_poisson_likelihood(D_T_j, lambda_E_j)
        psi_j = psi_j/np.sum(psi_j,axis=1)
...
class PoissonModelParameters(ModelParameters):
    ...
    def _update_phi(self, xi, psi):
        ...
        while phi_change > phi_stop:
            ...
            print(unnorm_complete_ll_left)
            print(unnorm_complete_ll_right)
...
class PoissonModelLikelihood(ModelLikelihood):
    ...
    def get_log_likelihood(self, parameters, priors):
        ...
        for j in range(0, J):
            ...
            if LOH_status_j == 'NONE':
	        log_likelihood_j = self._get_log_likelihood_by_segment_LN(parameters, priors, j)
    ...
    def _get_log_likelihood_by_segment_LN(self, parameters, priors, j):
        C = self.config_parameters['copynumber_tumor_num']
        G = self.config_parameters['genotypes_tumor_num']
        c_N = np.array(constants.COPY_NUMBER_NORMAL)
        c_T = np.array(self.config_parameters['copynumber_tumor'])
